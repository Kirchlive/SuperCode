#!/bin/bash
# /Users/rob/Development/SuperCode/SuperCode/test_scripts/test_design_command.sh
#
# This integration test verifies that the 'design' command correctly uses the
...
COMMAND_TO_RUN="bun run src/commands/design.ts"
LOG_FILE="$REPO_ROOT/test_results/test_design_command.log"
...
echo "--- Test: 'design' command integration ---" > "$LOG_FILE"
echo "Running command: $COMMAND_TO_RUN design --persona $PERSONA_ID 'a new API'" >> "$LOG_FILE"

# --- Execution ---
$COMMAND_TO_RUN design --persona "$PERSONA_ID" "a new API" >> "$LOG_FILE" 2>&1
EXIT_CODE=$?
