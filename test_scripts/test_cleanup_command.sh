#!/bin/bash
# /Users/rob/Development/SuperCode/SuperCode/test_scripts/test_cleanup_command.sh
#
# This integration test verifies that the 'cleanup' command correctly uses the
...
COMMAND_TO_RUN="bun run src/commands/cleanup.ts"
LOG_FILE="$REPO_ROOT/test_results/test_cleanup_command.log"
...
echo "--- Test: 'cleanup' command integration ---" > "$LOG_FILE"
echo "Running command: $COMMAND_TO_RUN cleanup --persona $PERSONA_ID 'unused variables'" >> "$LOG_FILE"

# --- Execution ---
$COMMAND_TO_RUN cleanup --persona "$PERSONA_ID" "unused variables" >> "$LOG_FILE" 2>&1
EXIT_CODE=$?
