// /Users/rob/Development/SuperCode/SuperCode/test_scripts/test_cleanup_command.ts
...
const LOG_FILE = path.join(REPO_ROOT, "test_results", "test_cleanup_command.log");
...
async function runTest(testCase: TestCase): Promise<boolean> {
    const command = `TEST_ENV=true bun run src/commands/cleanup.ts --prompt "${testCase.prompt}" ${testCase.args}`;
    let output = '';
    let passed = true;
...
async function main() {
    ...
    const testCases: TestCase[] = [
        {
            name: "Cleanup unused imports (dry run)",
            prompt: "find and remove unused imports",
            args: "--type imports --dry-run",
            expected: ["Detected Persona: refactorer", "Cleanup Type: imports", "Dry Run: true"],
            unexpected: [],
        },
        {
            name: "Aggressive code cleanup",
            prompt: "aggressively refactor the legacy module",
            args: "--type code --mode aggressive",
            expected: ["Detected Persona: refactorer", "Cleanup Type: code", "Mode: aggressive"],
            unexpected: [],
        },
    ];
