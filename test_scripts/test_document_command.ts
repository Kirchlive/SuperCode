// /Users/rob/Development/SuperCode/SuperCode/test_scripts/test_document_command.ts
...
const LOG_FILE = path.join(REPO_ROOT, "test_results", "test_document_command.log");
...
async function runTest(testCase: TestCase): Promise<boolean> {
    const command = `TEST_ENV=true bun run src/commands/document.ts --prompt "${testCase.prompt}" ${testCase.args}`;
    let output = '';
    let passed = true;
...
async function main() {
    ...
    const testCases: TestCase[] = [
        {
            name: "Document API (brief)",
            prompt: "the user service API",
            args: "--type api --style brief",
            expected: ["Detected Persona: scribe", "Documentation Type: api", "Style: brief"],
            unexpected: [],
        },
        {
            name: "Document Component (detailed)",
            prompt: "the main login component",
            args: "--type external --style detailed",
            expected: ["Detected Persona: scribe", "Documentation Type: external", "Style: detailed"],
            unexpected: [],
        },
    ];
