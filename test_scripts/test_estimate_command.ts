// /Users/rob/Development/SuperCode/SuperCode/test_scripts/test_estimate_command.ts
...
const LOG_FILE = path.join(REPO_ROOT, "test_results", "test_estimate_command.log");
...
async function runTest(testCase: TestCase): Promise<boolean> {
    const command = `TEST_ENV=true bun run src/commands/estimate.ts --prompt "${testCase.prompt}" ${testCase.args}`;
    let output = '';
    let passed = true;
...
async function main() {
    ...
    const testCases: TestCase[] = [
        {
            name: "Estimate time in days",
            prompt: "the new authentication feature",
            args: "--type time --unit days",
            expected: ["Detected Persona: architect", "Estimation Type: time", "Unit: days"],
            unexpected: [],
        },
        {
            name: "Estimate complexity",
            prompt: "the refactoring of the legacy database module",
            args: "--type complexity",
            expected: ["Detected Persona: refactorer", "Estimation Type: complexity"],
            unexpected: [],
        },
    ];
