// /Users/rob/Development/SuperCode/SuperCode/test_scripts/test_explain_command.ts
...
const LOG_FILE = path.join(REPO_ROOT, "test_results", "test_explain_command.log");
...
async function runTest(testCase: TestCase): Promise<boolean> {
    const command = `TEST_ENV=true bun run src/commands/explain.ts --prompt "${testCase.prompt}" ${testCase.args}`;
    let output = '';
    let passed = true;
...
async function main() {
    ...
    const testCases: TestCase[] = [
        {
            name: "Explain concept (advanced)",
            prompt: "the difference between REST and gRPC",
            args: "--level advanced",
            expected: ["Detected Persona: mentor", "Level: advanced"],
            unexpected: [],
        },
        {
            name: "Explain code (basic)",
            prompt: "this sorting algorithm",
            args: "--level basic --format examples",
            expected: ["Detected Persona: mentor", "Level: basic", "Format: examples"],
            unexpected: [],
        },
    ];
