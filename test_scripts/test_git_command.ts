// /Users/rob/Development/SuperCode/SuperCode/test_scripts/test_git_command.ts
...
const LOG_FILE = path.join(REPO_ROOT, "test_results", "test_git_command.log");
...
async function runTest(testCase: TestCase): Promise<boolean> {
    const command = `TEST_ENV=true bun run src/commands/git.ts --prompt "${testCase.prompt}" ${testCase.args}`;
    let output = '';
    let passed = true;
    ...
    const testCases: TestCase[] = [
        {
            ...
            prompt: "commit the latest changes",
            args: "--smart-commit",
            expected: ["Detected Persona: devops", "Smart Commit: true"],
            unexpected: [],
        },
    ];
