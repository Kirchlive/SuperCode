// /Users/rob/Development/SuperCode/SuperCode/test_scripts/test_improve_command.ts
...
const LOG_FILE = path.join(REPO_ROOT, "test_results", "test_improve_command.log");
...
async function runTest(testCase: TestCase): Promise<boolean> {
    const command = `TEST_ENV=true bun run src/commands/improve.ts --prompt "${testCase.prompt}" ${testCase.args}`;
    let output = '';
    let passed = true;
    ...
    const testCases: TestCase[] = [
        {
            ...
            prompt: "optimize the database query",
            args: "--type performance --safe",
            expected: ["Detected Persona: performance", "Improvement Type: performance", "Safe Mode: true"],
            unexpected: [],
        },
        {
            name: "Improve code quality",
            prompt: "refactor the user model for better readability",
            args: "--type quality",
            expected: ["Detected Persona: refactorer", "Improvement Type: quality"],
            unexpected: [],
        },
    ];
