// /Users/rob/Development/SuperCode/SuperCode/test_scripts/test_index_command.ts
...
const LOG_FILE = path.join(REPO_ROOT, "test_results", "test_index_command.log");
...
async function runTest(testCase: TestCase): Promise<boolean> {
    const command = `TEST_ENV=true bun run src/commands/index.ts --prompt "${testCase.prompt}" ${testCase.args}`;
    let output = '';
    let passed = true;
...
async function main() {
    ...
    const testCases: TestCase[] = [
        {
            name: "Index README",
            prompt: "create a new README for the project",
            args: "--type readme --format md",
            expected: ["Detected Persona: scribe", "Index Type: readme", "Format: md"],
            unexpected: [],
        },
    ];
