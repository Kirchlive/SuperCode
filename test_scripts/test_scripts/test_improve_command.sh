#!/bin/bash
# /Users/rob/Development/SuperCode/SuperCode/test_scripts/test_improve_command.sh
#
# This integration test verifies that the 'improve' command correctly uses the
...
COMMAND_TO_RUN="bun run src/commands/improve.ts"
LOG_FILE="$REPO_ROOT/test_results/test_improve_command.log"
...
echo "--- Test: 'improve' command integration ---" > "$LOG_FILE"
echo "Running command: $COMMAND_TO_RUN improve --persona $PERSONA_ID 'a slow function'" >> "$LOG_FILE"

# --- Execution ---
$COMMAND_TO_RUN improve --persona "$PERSONA_ID" "a slow function" >> "$LOG_FILE" 2>&1
EXIT_CODE=$?
