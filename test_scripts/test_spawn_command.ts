// /Users/rob/Development/SuperCode/SuperCode/test_scripts/test_spawn_command.ts
...
const LOG_FILE = path.join(REPO_ROOT, "test_results", "test_spawn_command.log");
...
async function runTest(testCase: TestCase): Promise<boolean> {
    const command = `TEST_ENV=true bun run src/commands/spawn.ts --prompt "${testCase.prompt}" ${testCase.args}`;
    let output = '';
    let passed = true;
...
async function main() {
    ...
    const testCases: TestCase[] = [
        {
            name: "Spawn parallel tasks",
            prompt: "refactor the UI and API layers",
            args: "--parallel",
            expected: ["Detected Persona: refactorer", "Parallel: true"],
            unexpected: [],
        },
    ];
