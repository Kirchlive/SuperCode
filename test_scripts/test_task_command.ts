// /Users/rob/Development/SuperCode/SuperCode/test_scripts/test_task_command.ts
...
const LOG_FILE = path.join(REPO_ROOT, "test_results", "test_task_command.log");
...
async function runTest(testCase: TestCase): Promise<boolean> {
    const command = `TEST_ENV=true bun run src/commands/task.ts --prompt "${testCase.prompt}" ${testCase.args}`;
    let output = '';
    let passed = true;
...
async function main() {
    ...
    const testCases: TestCase[] = [
        {
            name: "Create systematic task",
            prompt: "plan the database migration",
            args: "--strategy systematic",
            expected: ["Detected Persona: backend", "Strategy: systematic"],
            unexpected: [],
        },
    ];
