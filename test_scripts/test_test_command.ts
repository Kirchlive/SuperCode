// /Users/rob/Development/SuperCode/SuperCode/test_scripts/test_test_command.ts
...
const LOG_FILE = path.join(REPO_ROOT, "test_results", "test_test_command.log");
...
async function runTest(testCase: TestCase): Promise<boolean> {
    const command = `TEST_ENV=true bun run src/commands/test.ts --prompt "${testCase.prompt}" ${testCase.args}`;
    let output = '';
    let passed = true;
    ...
    const testCases: TestCase[] = [
        {
            ...
            prompt: "run all tests for the project and generate coverage",
            args: "--type all --coverage",
            expected: ["Detected Persona: qa", "Test Type: all", "Coverage: true"],
            unexpected: [],
        },
        {
            name: "Run e2e tests in watch mode",
            prompt: "watch the e2e tests",
            args: "--type e2e --watch",
            expected: ["Detected Persona: qa", "Test Type: e2e", "Watch Mode: true"],
            unexpected: [],
        },
    ];
