// /Users/rob/Development/SuperCode/SuperCode/test_scripts/test_troubleshoot_command.ts
...
const LOG_FILE = path.join(REPO_ROOT, "test_results", "test_troubleshoot_command.log");
...
async function runTest(testCase: TestCase): Promise<boolean> {
    const command = `TEST_ENV=true bun run src/commands/troubleshoot.ts --prompt "${testCase.prompt}" ${testCase.args}`;
    let output = '';
    let passed = true;
...
async function main() {
    ...
    const testCases: TestCase[] = [
        {
            name: "Troubleshoot build error",
            prompt: "the build is failing with a type error",
            args: "--type build --trace",
            expected: ["Detected Persona: analyzer", "Issue Type: build", "Trace: true"],
            unexpected: [],
        },
    ];
