// /Users/rob/Development/SuperCode/SuperCode/test_scripts/test_workflow_command.ts
...
const LOG_FILE = path.join(REPO_ROOT, "test_results", "test_workflow_command.log");
...
async function runTest(testCase: TestCase): Promise<boolean> {
    const command = `TEST_ENV=true bun run src/commands/workflow.ts --prompt "${testCase.prompt}" ${testCase.args}`;
    let output = '';
    let passed = true;
...
async function main() {
    ...
    const testCases: TestCase[] = [
        {
            name: "Agile workflow for frontend",
            prompt: "a new user dashboard for the frontend",
            args: "--strategy agile --output tasks",
            expected: ["Detected Persona: frontend", "Strategy: agile", "Output: tasks"],
            unexpected: [],
        },
    ];
